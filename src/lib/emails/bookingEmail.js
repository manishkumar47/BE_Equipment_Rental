...
const buildBookingContext = ({ user, equipment, booking, }) => ({
    user,
    equipment,
    booking: {
        ...booking,
...
export const sendBookingComplete = async ({ user, equipment, booking, }) => {
    try {
...
export const sendBookingReminder = async ({ user, equipment, booking, }) => {
    try {
        const remainingMinutes = Math.max(0, Math.round((booking.rentTo.getTime() - new Date().getTime()) / (1000 * 60)));
        const remainingHours = Math.floor(remainingMinutes / 60);
        const remainingMins = remainingMinutes % 60;
        const remainingString = remainingHours > 0
        ...
        logger.info(`Reminder email sent to ${user.email} for booking ${booking.id}`);
        return true;
    }
    catch (error) {
