...
export const sendReminderEmail = async () => {
    try {
        const pendingBookings = await rentalBookingService.getPendingReminderBookings();
        if (pendingBookings.length === 0) {
            return;
        }
        const now = new Date();
        for (const rentalBooking of pendingBookings) {
            if (rentalBooking.isReminderSent)
                continue;
            ...
            if (now < rentFromDate || now > rentToDate)
                continue;
            if (!rentalBooking.user || !rentalBooking.equipment)
                continue;
            const totalDurationInMins = (rentToDate.getTime() - rentFromDate.getTime()) / (1000 * 60);
            const minutesRemaining = (rentToDate.getTime() - now.getTime()) / (1000 * 60);
            ...
            if (minutesRemaining <= targetBufferInMins) {
                logger.info(`🔔 Triggering Email for Booking ID ${rentalBooking.id}. ` +
                ...
                if (reminderSent) {
                    ...
                    logger.info(`Reminder sent and marked for booking ${rentalBooking.id}`);
                }
                else {
                    logger.warn(`Reminder email failed for booking ${rentalBooking.id}; not marking reminder sent.`);
                }
            }
        }
    }
